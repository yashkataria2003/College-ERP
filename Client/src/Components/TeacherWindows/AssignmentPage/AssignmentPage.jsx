...
const AddAssignmentForm = ({ collectingData }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:4000/api/v1/teacher/assignments', {
                method: 'POST',
                body: formData,
            });
    ...
    return (
        <div className='flex flex-col justify-center items-center w-[40vw] h-[90vh] gap-y-2'>
            <h2 className='text-[#000080] text-[1.8rem] font-bold'>Add Assignment Details</h2>
            <form
                action=""
                method='post'
...
const EditAssignmentForm = ({ data, collectingData }) => {
    const navigate = useNavigate();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:4000/api/v1/teacher/assignments/${_id}`, {
                method: 'PUT',
                headers: {
    ...
    return (
        <div className='flex flex-col justify-center items-center w-[40vw] h-[90vh] gap-y-2'>
            <h2 className='text-[#000080] text-[1.8rem] font-bold'>Update Assignment</h2>
            <form
                action=""
                method='post'
                className='flex flex-col justify-between items-center py-[5vh] h-fit gap-y-[8vh] w-[80%] border-[#000080] border-2 rounded-xl shadow-[5px_5px_0px_0px_rgba(64,63,63)] bg-[#fff]'
            ...
            >
                <ul className='flex flex-col justify-center items-center w-[100%] h-[100%] gap-y-[3vh] px-1'>
                    <iframe
                        ...
                        src={`http://localhost:4000/Assignments/${attachment}`}
                    />
                    <li className='flex justify-end items-center w-[85%] gap-x-[1vw]'>
                        <label htmlFor="title" className='text-[#000080] text-[1.2rem] hover:cursor-pointer'>Title</label>
                        <input
...
const DeleteAssignmentForm = ({ data, collectingData }) => {
    const navigate = useNavigate();
    ...
    return (
        <div className='flex flex-col justify-center items-center w-[40vw] h-[90vh] gap-y-2'>
            <h2 className='text-[#000080] text-[1.8rem] font-bold'>Delete Assignment Details</h2>
            <form
                action=""
                method='post'
...
const AssignmentCard = ({ data, handleEditClick, handleDeleteClick }) => {
    const { _id, title, subject, attachment, course, section, dueDate } = data;
...
const Assignments = ({ data, title, handleEditClick, handleDeleteClick }) => {
    if (data?.length > 0) {
        // console.log(data)
...
const AssignmentPage = () => {
    ...
    return (
        <div className='flex justify-start items-center w-[80vw] h-[90vh]'>
            {
                addForm && <AddAssignmentForm collectingData={collectingData} />
            }
            {
                editForm && <EditAssignmentForm data={editFormData} collectingData={collectingData} />
            }
            {
                deleteForm && <DeleteAssignmentForm data={deleteFormData} collectingData={collectingData} />
            }
