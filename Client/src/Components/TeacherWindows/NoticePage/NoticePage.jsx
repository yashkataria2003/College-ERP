...
const AddNoticeForm = ({ collectingData }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            formData.append('course', noticeData.course);
            formData.append('section', noticeData.section);
            formData.append('attachment', noticeData.attachment);

            const response = await fetch('http://localhost:4000/api/v1/teacher/notices', {
                method: 'POST',
                body: formData,
            });
    ...
    return (
        <div className='flex flex-col justify-center items-center w-[40vw] h-[90vh] gap-y-2'>
            <h2 className='text-[#000080] text-[1.8rem] font-bold'>Add Notice Details</h2>
            <form
                action=""
                method='post'
                className='flex flex-col justify-between items-center py-[5vh] h-fit gap-y-[8vh] w-[80%] border-[#000080] border-2 rounded-xl px-[1vw] shadow-[5px_5px_0px_0px_rgba(64,63,63)] bg-[#fff]'
...
const EditNoticeForm = ({ data, collectingData }) => {
    const { _id, title, description, course, section, attachment } = data;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:4000/api/v1/teacher/notices/${_id}`, {
                method: 'PUT',
                headers: {
    ...
    return (
        <div className='flex flex-col justify-center items-center w-[40vw] h-[90vh] gap-y-2'>
            <h2 className='text-[#000080] text-[1.8rem] font-bold'>Update Notice</h2>
            <form
                action=""
                method='post'
                className='flex flex-col justify-between items-center py-[5vh] h-fit gap-y-[8vh] w-[80%] border-[#000080] border-2 rounded-xl px-[1vw] shadow-[5px_5px_0px_0px_rgba(64,63,63)] bg-[#fff]'
            ...
            >
                <ul className='flex flex-col justify-center items-center w-[100%] h-[100%] gap-y-[3vh] px-1'>
                    <iframe
                        ...
                        src={`http://localhost:4000/StudentNotices/${attachment}`}
                    />
                    <li className='flex justify-end items-center w-[85%] gap-x-[1vw]'>
                        <label htmlFor="title" className='text-[#000080] text-[1.2rem] hover:cursor-pointer'>Title</label>
                        <input
                    ...
                    <li className='flex justify-end items-center w-[85%] gap-x-[1vw]'>
                        <label htmlFor="title" className='text-[#000080] text-[1.2rem] hover:cursor-pointer'>Course</label>
                        <input
                            type="text"
                            name="course"
                            key="course"
                    ...
                    <li className='flex justify-end items-center w-[85%] gap-x-[1vw]'>
                        <label htmlFor="title" className='text-[#000080] text-[1.2rem] hover:cursor-pointer'>Section</label>
                        <input
                            type="text"
                            name="section"
                            key="section"
...
const DeleteNoticeForm = ({ data, collectingData }) => {
    const navigate = useNavigate()
    const { _id, attachment, title, description, course, section } = data;
    ...
    return (
        <div className='flex flex-col justify-center items-center w-[40vw] h-[90vh] gap-y-2'>
            <h2 className='text-[#000080] text-[1.8rem] font-bold'>Delete Notice</h2>
            <form
                action=""
                method='post'
                className='flex flex-col justify-center items-center py-[5vh] h-fit gap-y-[5vh] w-[80%] border-[#000080] border-2 rounded-xl px-[1vw] shadow-[5px_5px_0px_0px_rgba(64,63,63)] bg-[#fff]'
                encType="multipart/form-data"
                onSubmit={handleSubmit}
            >
...
const NoticeCard = ({ data, handleEditClick, handleDeleteClick }) => {
    const { _id, title, description, attachment, course, section } = data;
    ...
    return (
        <div className="flex flex-col justify-center items-center min-h-max w-[30vw] rounded-xl p-1">
            <div className="flex justify-between items-center h-fit w-[100%] bg-[#fff] rounded-t-md py-3 px-2 border-[0.2vh] border-black">
                <div className='flex flex-col justify-center items-start w-[95%] h-auto gap-y-[3vh]'>
                    <div className='flex flex-col justify-start items-start w-[100%]'>
                        <p className=''><strong>{title}</strong></p>
                        <p className='text-wrap'>{description}</p>
                    </div>
                    <strong>Course - <span className='text-[#000080]'>{course}</span></strong>
                </div>
            ...
            <div className="flex justify-center items-center min-h-max w-[100%] bg-[#9d1212] rounded-b-xl border-t-[0.1vh] border-t-white h-[40vh]">
                <iframe
                    className="h-[100%] w-[100%]"
                    src={`http://localhost:4000/StudentNotices/${attachment}`}
                />
