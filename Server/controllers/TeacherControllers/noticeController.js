import noticeModel from '../../models/TeacherModels/noticeSchema.js'
import fs from 'fs';
...
export const addNotice = async(req, res)=> {
    try {
        const { title, description, course, section } = await req.body;
        ...
        await res.status(200).json({success:true, data: notice, message:"Student Notice Saved"})
    } 
...
export const removeNotice = async(req, res) => {
    try {
        ...
        if (notice.attachment) {
            ...
            const noticePath = path.join(process.cwd(), 'public', 'StudentNotices', notice.attachment);
        ...
        await res.status(200).json({success:"true", messege:"Student Notice deleted Successfully"})
    } 
